...
def parse_args():
    parser = argparse.ArgumentParser(description="Testing script for speech model with adapter")
...
def main():
    ...
    if device == 'cuda' and torch.cuda.is_available():
        ...
        llm_embed_layer = llm_embed_layer.to(device)    
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=(device == 'cuda')):
            ...
            labels = text_outputs[:, 1:seq_len + 1].to(device)
    ...
    print("Оценка качества закончена.")
