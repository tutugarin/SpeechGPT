...
class AlpacaItem:
    ...
    Атрибуты:
        instruction (str): описывает задачу, которую должна выполнить модель. Каждая из 52 тысяч инструкций уникальна.
        input (str): необязательный контекст или ввод для задачи. 
                    Например, если инструкция — "Суммируйте следующую статью", то вводом является сама статья.
